main:
	...
	andi $t1, $t0, 0x00FF # mask 0th byte and store in register $t1
	andi $t2, $t0, 0xFF00 # mask 1st byte and store in register $t2
	srl $t3, $t0, 16 # shift right 2nd and 3rd bytes by 16 bits, will occupy positions of original 0th and 1st byte resepectively
	andi $t4, $t3, 0x00FF # mask 2nd byte (technically in 0th byte spot) and store in register $t4
	andi $t5, $t3, 0xFF00 # mask 3rd byte (technically in 1ST byte spot) and store in register $t5

	srl $t6, $t5, 8 # shift masked byte 3 into original position of byte 0 
	sll $t7, $t4, 8 # shift masked byte 2 into original position of byte 1
